...
Voice statement: 평서체로 작성한다. 전문가가 동료에게 말하는 톤으로, 학술적이며 간결하다. 1인칭 주어는 생략하고, 헷징 표현 ("~할 것 같다", "~로 보인다", "should", "probably")은 쓰지 않는다. 영어 학술 용어는 한글 음차에 괄호 영문을 병기하되, 자주 쓰이는 약어 (RNA-seq, PCR, ALE, TSS)는 그대로 둔다. em dash (—)는 사용하지 않으며 쉼표, 콜론, 괄호로 분리한다.
